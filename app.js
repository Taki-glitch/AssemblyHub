...
async function bindAdminInteractions() {
  ...
  document.querySelectorAll('[data-user-action]').forEach((button) => button.addEventListener('click', async () => {
    ...
    if (button.dataset.userAction === 'reset') {
      try {
        const link = await resetPasswordSecurely(user);
        authState.notice = link
          ? `Lien de réinitialisation généré pour ${user.email} : ${link}`
          : `Email de réinitialisation envoyé à ${user.email}.`;
      } catch (error) {
        authState.error = `Réinitialisation impossible : ${friendlyFirebaseError(error)}`;
      }
      renderRoute();
      return;
    }
    if (button.dataset.userAction === 'toggle') {
      ...
      const confirmMessage = willDisable
        ? `Désactiver l'accès de ${user.email} ?`
        : `Réactiver l'accès de ${user.email} ?`;
      if (!confirm(confirmMessage)) return;
      try {
        if (willDisable) {
          await disableUserSecurely(user);
          authState.notice = 'Utilisateur désactivé.';
        } else {
          await enableUserSecurely(user);
          authState.notice = 'Utilisateur réactivé.';
        }
      } catch (error) {
        authState.error = `Action impossible : ${friendlyFirebaseError(error)}`;
      }
    }
    if (button.dataset.userAction === 'delete') {
      ...
      try {
        ...
        authState.notice = 'Utilisateur supprimé.';
      } catch (error) {
        authState.error = `Suppression impossible : ${friendlyFirebaseError(error)}`;
      }
    }
    await loadUsers();
    renderRoute();
...
async function disableUserSecurely(user) {
  try {
    const callable = firebaseApi.functionsModule.httpsCallable(firebaseApi.functions, 'disableAssemblyHubUser');
    await callable({ uid: user.uid });
    return;
  } catch (error) {
    if (!String(error?.code || '').includes('functions/not-found')) throw error;
  }
  // Repli client (fonction non déployée) : Firestore uniquement, le compte Auth reste actif.
  const { doc, updateDoc } = firebaseApi.firestoreModule;
  await updateDoc(doc(firebaseApi.db, 'users', user.uid), { active: false });
}

async function enableUserSecurely(user) {
  try {
    const callable = firebaseApi.functionsModule.httpsCallable(firebaseApi.functions, 'enableAssemblyHubUser');
    await callable({ uid: user.uid });
    return;
  } catch (error) {
    if (!String(error?.code || '').includes('functions/not-found')) throw error;
  ...
  const { doc, updateDoc } = firebaseApi.firestoreModule;
  await updateDoc(doc(firebaseApi.db, 'users', user.uid), { active: true });
}

async function resetPasswordSecurely(user) {
  try {
    const callable = firebaseApi.functionsModule.httpsCallable(firebaseApi.functions, 'resetAssemblyHubUserPassword');
    const result = await callable({ uid: user.uid });
    return result?.data?.resetLink || null;
  } catch (error) {
    if (!String(error?.code || '').includes('functions/not-found')) throw error;
  }
  // Repli client (fonction non déployée) : envoi direct de l'email Firebase standard.
